...
idf=calidf(pop)
famacc=calidf_ebv_gv(pop_test,trait=1)####!!!!!!We set trait=1 to calculate fac of dbs
}else{
...
}else if(sch%in%c("ds","tds")){
  output$accuracy[g+1] <- cor(pop_gsc@ebv,pop_gsc@gv[,2]) 
}else if(sch%in%c("dbs","tdbs")){
  output$accuracy[g+1] <- cor(pop_gsc@ebv,pop_gsc@gv[,1]) ##!!!!!! mention above
}