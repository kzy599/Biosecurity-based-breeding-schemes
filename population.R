...
#simple population, if you don't want to use the genome information of Pacific white shrimp
...
SP$addTraitA(100,mean=c(0,0),var = c(5.39,5.39),corA = matrix(c(1,(mGEI/10),(mGEI/10),1),nrow = 2))
SP$setVarE(h2=c(0.41,0.41))#遗传力和偏差
SP$addSnpChip(1250,minSnpFreq = 0.05)#55kSNP芯片
...
pop <- selectCross(pop_founder,
                   ...
                   nCrosses = nfamily,nProgeny = nProgeny,
                   use = "rand",
                   simParam = SP)
save.image(file = FounderG0)
